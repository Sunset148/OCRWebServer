...
std::vector<TextLine> OcrLite::detect(const char *path, const char *imgName,
...
std::vector<TextLine> OcrLite::detect(const cv::Mat& mat, int padding, int maxSideLen, float boxScoreThresh, float boxThresh, float unClipRatio, bool doAngle, bool mostAngle)
{
    cv::Mat originSrc;
...
//    Logger("FullDetectTime(%fms)\n", fullTime);
    ...
    for(int i=0;i<textLines.size();i++){
        result.push_back(textLines[i].text);
    }
