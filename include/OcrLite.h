...
public:
    ...
    std::vector<TextLine> detect(const char *path, const char *imgName,
                     int padding, int maxSideLen,
                     float boxScoreThresh, float boxThresh, float unClipRatio, bool doAngle, bool mostAngle);

    std::vector<TextLine> detect(const cv::Mat &mat,
                     int padding, int maxSideLen,
                     float boxScoreThresh, float boxThresh, float unClipRatio, bool doAngle, bool mostAngle);
    inline std::vector<std::string> getResult(){
        return result;
    }
private:
    ...
    AngleNet angleNet;
    CrnnNet crnnNet;
    std::mutex loggerMutex; // 定义一个互斥量用于保护日志输出操作
