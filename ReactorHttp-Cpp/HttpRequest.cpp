...
bool HttpRequest::parseRequestBody(Buffer* readBuf)
{
    ...
    if (!readBuf->getImage().empty())
    {
        m_image = readBuf->getImage();
        readBuf->readPosIncrease(readBuf->readableSize());
        // 请求头被解析完了, 跳过空行
        // 修改解析状态
        // 忽略 post 请求, 按照 get 请求处理
        setState(PrecessState::ParseReqDone);
...
bool HttpRequest::processHttpRequest(HttpResponse* response)
{
    if (strcasecmp(m_method.data(), "get") != 0)
    {
        if (!m_image.empty()) {
            ...
            PredString predString;
            vector<string> reconResult = predString.getPredString(m_image);
            Debug("解析完毕,URL:%s", m_url.c_str());
            int size = m_requestContent.size();
            for (int i = 0; i < size; i++)
                if (m_url.find(m_requestContent[i]) != string::npos) {
                    if (i == 0) {
                        IDRecognition idRecognition(reconResult);
                        response->setFileName(idRecognition.toJson());
                        break;
                    } else if(i == 1){
                        DigitalRecognition digitalRecognition(reconResult);
                        response->setFileName(digitalRecognition.toJson());
                    }else{
                        BankIdentification bankIdentification(reconResult);
                        response->setFileName(bankIdentification.toJson());
                    }
                }
...
void HttpRequest::sendJson(string JsonData, Buffer* sendBuf,int cfd)
{
    sendBuf->appendString(JsonData);
//    Debug("JsonData:%s",JsonData.c_str());
//    Debug("sendData:%s",sendBuf->data());
